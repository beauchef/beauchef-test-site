...
function ATRK() {
	...
	this.linkParams = function() {
		...
		var atrk_p = this._getFromPaidClickCookie();
		atrk_p = (atrk_p == '-') ? '' : "&ATRK_p=" + atrk_p;
		var p = "ATRK_y=" + this._getCookie('ATRK_y') + "&ATRK_a=" + this._getCookie('ATRK_a') + atrk_x + "&9ckey=" + this.clientKey + atrk_p;
    ...
    this.getCallbackUrl = function() {
        return this._getParamFromLink('atrkcb');
    };

	this.replacePhoneNumbers = function() {
	    // Make sure to initialize the cookie.
        var fromPaidClick = this._initFromPaidClickCookie();

        if (fromPaidClick == '1' && this._getPhoneMappingsKeys().length > 0) {
            this._cleanPhoneMappingsKeys();
            this._domReady(function() {
                ATRKtracker._replacePhoneNumbers();
            });
        }
    };
    
    this.invokeCallback = function(callbackUrl) {
        this._domReady(function() {
            ATRKtracker._invokeCallback(callbackUrl);
        });
    };
    ...
    this._domReady = function(callback) {
        var ready = false;
    ...
    this._cleanPhoneMappingsKeys = function() {
	    var keys = this._getPhoneMappingsKeys();
	    for (var i = 0; i < keys.length; i++) {
	        var cleanKey = keys[i].replace(new RegExp("[\\)\\(\\. -]+", "g"), "");
	        this.phoneMappings[cleanKey] = this.phoneMappings[keys[i]];
	    }
	}

	this._loadPhoneMappings = function(providedPhoneMappings) {
        for (var key in providedPhoneMappings) {
            if (providedPhoneMappings.hasOwnProperty(key) && (typeof providedPhoneMappings[key]) == "string") {
                this.phoneMappings[key] = providedPhoneMappings[key];
            }
        }
	}

	this._getPhoneMappingsKeys = function() {
	    var keys = [];
	    for(var k in this.phoneMappings) {
	        if (this.phoneMappings.hasOwnProperty(k)) {
	            keys.push(k);
            }
	    }
	    return keys;
	...
	this._getFromPaidClickCookie = function() {
	    return this._getCookie('ATRK_p');
	}

	// Initialize the fromPaidClick cookie if it is not done yet
	this._initFromPaidClickCookie = function() {
        var atrk_p = this._getParamFromLink('ATRK_p');
		// Try to extract params from the URL
		// and set cookies as needed
		if (atrk_p !== '-') {
		    this._getDocument().cookie = 'ATRK_p=' + atrk_p + ';path=/;expires=' + this.neverExpires + ';domain=' + this.clientDomain;
		}

		var fromPaidClick = this._getFromPaidClickCookie();
		if (fromPaidClick === '-' || fromPaidClick === '0') {
		    var trackingNumber = this._getParamFromLink('atrkid');
		    if (trackingNumber !== "-") {
		        fromPaidClick = '1';
		        this._getDocument().cookie = 'ATRK_p=' + fromPaidClick + ';path=/;expires=' + this.neverExpires + ';domain=' + this.clientDomain;
		    }
		}
		return fromPaidClick;
	}

	this._replacePhoneNumbers = function() {
        // Replace node with class name 'atrkPhoneNumber'
        var phoneNumbers = this._getDocument().getElementsByClassName("atrkPhoneNumber");
        for (var i = 0; i < phoneNumbers.length; i++) {
            this._replacePhoneNumber(phoneNumbers[i]);
        }

        // Replace link where the HREF is a phone number
        var links = this._getDocument().getElementsByTagName("a");
        for (var i = 0; i < links.length; i++) {
            if (links[i].href !== undefined && links[i].href.substr(0, "tel://".length) == "tel://") {
                this._replacePhoneNumber(links[i]);
            }
        }
	}

    this._replacePhoneNumber = function(phoneNumberNode) {
        var phoneNumber = phoneNumberNode.innerText.replace(new RegExp("[\\)\\(\\. -]+", "g"), "");
        var newPhoneNumber = this.phoneMappings[phoneNumber];
        if (newPhoneNumber !== undefined) {
            phoneNumberNode.innerText = newPhoneNumber;
            if (phoneNumberNode.href !== undefined) {
                phoneNumberNode.href = "tel://" + newPhoneNumber;
            }
        }
    }

	this._invokeCallback = function(url) {
		this.logger('in _invokeCallback(url=' + url + ')');
		if (url.indexOf('{clientId}') < 0) {
			this.logger('  There was no {clientId} to replace in the url: ' + url);
		} else {
			var callbackUrl = url.replace('{clientId}', this.clientKey);
			this.logger('  Final callback URL: ' + callbackUrl);
			var client = new HttpClient();
			client.get(callbackUrl, function(response) {
				// Only log the response. No need to do anything.
				ATRKtracker.logger('  Callback response: ' + response);
			});
		}
	};
	...
	this._gifImagePull = function(tag, conversion) {
		...
		if ((typeof this.ttdAdvertiserId != 'undefined') && (this.ttdAdvertiserId != '')) {
			...
			if (tag == 'atrk' || this.pageLocation.href.indexOf('atrkid') != -1) {
			    ...
			    path = this.protocol + 'match.adsrvr.org/track/cmf/generic?ttd_pid=acquisio&ttd_tpi=1&ttd_puid='+this._urlEncode(this._urlEncode(path));
			}
		}
...
// HttpClient to make GET calls for callbacks
...
try {
	...
	if ((typeof _atrkpm != 'undefined') && _atrkpm.constructor == Object) {
	    ATRKtracker._loadPhoneMappings(_atrkpm);
	}

	// Phone remplacement service
	ATRKtracker.logger('Call phone remplacement service');
    ATRKtracker.replacePhoneNumbers();
    
    // Invoke callback if there is one to call
	ATRKtracker.logger('Check if we have a callback to call...');
    var callbackUrl = ATRKtracker.getCallbackUrl();
    if ((typeof callbackUrl != 'undefined') && (callbackUrl != '-')) {
        ATRKtracker.invokeCallback(callbackUrl);
    } else {
        ATRKtracker.logger('No URL to callback');
    }
